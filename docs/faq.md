...
* Full type-checked with `mypy`

Note that ther are a few features present in `repo` that are missing from `tsrc`
...
Last but not least, with `tsrc` you do everything with commands like `tsrc init` and `tsrc sync`, or simple `yaml` files,  which is musch easier than using the `git submodule`. CLI.
